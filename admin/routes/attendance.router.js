...
attendanceRouter.get('/mark', async (req, res, next) => {
  const { ugnumber, currentDateTimeISO } = req.query;
  ...
  if (!currentDateTimeISO) {
    res.attendanceRender('admin-attendance', {
      err: true,
      message: 'Invalid Current Date TIme ISO'
    });
    return;
  }
  ...
  try {
    ...
    if (dateNow.equals(lastDate)) {
      ...
      if (subType === 'single' && lastCount === 1) {
        ...
        res.attendanceRender('admin-attendance', {
          success: true,
          message: `Quota Over for ${ugnumber.toString()} and Attanced marked from next day`,
        });
        return;
      }
      ...
      if (subType === 'both') {
        ...
        console.log({ lunchTime: lunchTime.toISOTime(), dinnerTime: dinnerTime.toISOTime(), dateTimeNow: dateTimeNow.toISOTime() });
        console.log();
        if ((lunchTime <= dateTimeNow) && (dateTimeNow <= dinnerTime) && lastCount == 1) {
          res.attendanceRender('admin-attendance', {
            modal: true,
            message: 'Already Attendance Mark for Lunch time for ' + ugnumber,
          });
